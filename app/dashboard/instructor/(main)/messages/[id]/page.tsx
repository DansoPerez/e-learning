...
export default async function InstructorConversationPage({
  ...
  if (!conversation || conversation.type !== "STUDENT_INSTRUCTOR") notFound();
  const allowed = await canAccessConversation(user.id, user.role, conversation);
  if (!allowed) notFound();

  return (
    <InstructorDashboardWrapper title="Conversation">
      <Link
      ...
      <div className="surface-card mb-4 p-4">
        <p className="font-bold">{conversation.student.name ?? conversation.student.email}</p>
        {conversation.course ?
